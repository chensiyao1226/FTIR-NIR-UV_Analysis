...
#=========cut================
Edge1L = 800; Edge1Lnum=100000 ##minEdge
Edge1H = 1200; Edge1Hnum=1 ##minEdge
Edge2L = 1000; Edge2Lnum=100000 ##maxEdge
Edge2H = 1400; Edge2Hnum=1 ##maxEdge
...
plt.figure()
for i in range(int(counter/rep_t)):
    plt.plot(norm_cutted_data[0, :], sum_cutted_data[i], label=i+6, alpha=0.6)
plt.legend()
...
for i in range(1,content_cutted.shape[0]):
    savgol[i]=savgol_filter(content_cutted[i], window_length = 29, polyorder = 5, deriv = 2)
    savgol[i,0:35]=0
...
plt.figure()
for i in range(int(counter/rep_t)):
    plt.plot(norm_data[0, :], sum_data[i], label=i+6, alpha=0.4)
plt.legend()
plt.xlabel('Wave Number (cm-1)')
