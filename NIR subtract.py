...
for root, dirs, files in os.walk(path_root):
    for filename in files:
        if ('.csv' in filename):
            ...
            samp_dat = samp_sheet.iloc[0:4620, 0:2]
...
#=========cut================
Edge1L = 900; Edge1Lnum=1 ##minEdge
Edge1H = 990; Edge1Hnum=100000 ##minEdge
Edge2L = 920; Edge2Lnum=1 ##maxEdge
Edge2H = 1800; Edge2Hnum=100000 ##maxEdge
...
for i in range(int(counter/rep_t)):
    ...
    if(lab>4):
        lab+=2
    plt.plot(norm_cutted_data[0, :], sum_cutted_data[i], label=lab)
plt.legend()
...
for i in range(1,content_cutted.shape[0]):
    savgol[i]=savgol_filter(content_cutted[i], window_length = 79, polyorder = 2, deriv = 2)
    savgol[i,0:50]=0
    savgol[i,(len(savgol[0])-50):(len(savgol[0]))] = 0
...
plt.figure()
plt.plot(norm_data[0, :], PC, color='red', alpha=0.4)

indicesVH = signal.find_peaks(PC, height=0.05)
indicesVL = signal.find_peaks(-PC, height=0.02)

for k in range(len(indicesVH[0])):
    nVH = indicesVH[0][k]
    peakH.append((round(norm_data[0][nVH]), PC[nVH]))
    plt.plot(norm_data[0][nVH], PC[nVH], "x")
    strH=str(peakH[k][0])+":"+str(peakH[k][1])
    print(strH)
for k in range(len(indicesVL[0])):
    nVL = indicesVL[0][k]
    peakL.append((round(norm_data[0][nVL]), PC[nVL]))
    plt.plot(norm_data[0][nVL], PC[nVL], "x")
...
label = "PC"+str(i)
print(label)
save(path_root, "peak record NIR", label, peakH)
save(path_root, "peak record NIR", label, peakL)
...
for i in range(int(counter/rep_t)):
    ...
    if(lab>4):
        lab+=2
    plt.plot(norm_data[0, :], sum_data[i], label=lab)
plt.legend()
plt.xlabel('Wave Length (nm)')
...
for i in range(int(counter/rep_t)):
    ...
    labelP = "PCALL"+str(i)
    save(path_root, "peak record NIR", labelP, peakPH)
    save(path_root, "peak record NIR", labelP, peakPL)
    ...
    save(path_root, "peak record NIR", labelD, peakDH)
    save(path_root, "peak record NIR", labelD, peakDL)
