...
#=========cut================
Edge1L = 217; Edge1Lnum=1 ##minEdge
Edge1H = 348; Edge1Hnum=100000 ##minEdge
Edge2L = 352; Edge2Lnum=1 ##maxEdge
Edge2H = 748; Edge2Hnum=100000 ##maxEdge
...
for i in range(1,content_cutted.shape[0]):
    savgol[i]=savgol_filter(content_cutted[i], window_length = 29, polyorder = 2, deriv = 2)
    savgol[i,0:30]=0
    savgol[i,(len(savgol[0])-30):(len(savgol[0]))] = 0
...
for i in range(princ_amount):
    ...
    label = "PC"+str(i)
    print(label)
    save(path_root, "peak record UV", label, peakH)
    save(path_root, "peak record UV", label, peakL)
...
for i in range(int(counter/rep_t)):
    ...
    labelP = "PCALL"+str(i)
    save(path_root, "peak record UV", labelP, peakPH)
    save(path_root, "peak record UV", labelP, peakPL)
    ...
    save(path_root, "peak record UV", labelD, peakDH)
    save(path_root, "peak record UV", labelD, peakDL)
